...
function FresagemPage() {
  return (
    <>
      <PageHeader eyebrow="Serviço especializado" title="Fresagem Asfáltica" description="Remoção controlada do pavimento asfáltico com equipamentos de última geração e equipe técnica especializada." />
        ...
        <div className="mx-auto max-w-7xl px-6 grid gap-12 lg:grid-cols-2 items-center">
          <div>
            ...
            <div className="mt-5 space-y-4 text-muted-foreground leading-relaxed">
              <p>A fresagem asfáltica é o processo de remoção controlada e precisa de camadas do pavimento existente, utilizando fresadoras especializadas equipadas com tambores rotativos dotados de bits de tungstênio.</p>
              <p>É a etapa fundamental antes de qualquer recapeamento ou reabilitação de pavimentos, garantindo perfeita aderência da nova camada e nivelamento adequado da pista.</p>
              <p>Na West Fresa, executamos fresagem em rodovias, vias urbanas, estacionamentos, pátios industriais e obras de grande porte, com controle milimétrico de profundidade e largura.</p>
            </div>
          </div>
          <div className="relative">
            <img src={milling} alt="Fresagem asfáltica em operação" className="rounded-2xl shadow-2xl object-cover aspect-[4/3] w-full" />
          </div>
        </div>
      </section>
          ...
          <div className="grid gap-4 grid-cols-2 lg:grid-cols-3">
            {[millingAction, machineCloseup, transport, loading, milling].map((img, i) => (
              <div key={i} className={`overflow-hidden rounded-xl shadow-md ${i === 0 ? "col-span-2 row-span-2" : ""}`}>
                <img src={img} alt={`Operação ${i + 1}`} className="h-full w-full object-cover aspect-[4/3] hover:scale-105 transition-transform duration-500" />
              </div>
            ))}
          </div>
